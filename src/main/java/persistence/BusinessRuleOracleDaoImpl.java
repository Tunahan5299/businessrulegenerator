...
import persistence.BusinessRuleOracleDao;
...
public class BusinessRuleOracleDaoImpl extends OracleBaseDao implements BusinessRuleOracleDao {
    ...
    public BusinessRule save(BusinessRule b) {
        try {
            PreparedStatement pstmnt = OracleBaseDao.conn.prepareStatement("INSERT INTO businessrule (naam, type, omschrijving, foutmelding, status) VALUES (?, ?, ?, ?, ?)");
    ...
    public BusinessRule update(BusinessRule b) {
        try {
            PreparedStatement pstmnt = OracleBaseDao.conn.prepareStatement("UPDATE businessrule SET naam = ?, type = ?, omschrijving = ?, foutmelding = ?, status = ? WHERE naam = ?");
            pstmnt.setString(1, b.getNaam());
            pstmnt.setString(2, b.getType());
            pstmnt.setString(3, b.getOmschrijving());
            pstmnt.setString(4, b.getFoutmelding());
    ...
    public boolean delete(BusinessRule b) {
        try {
            PreparedStatement pstmnt = OracleBaseDao.conn.prepareStatement("DELETE FROM businessrule WHERE naam = ?");
            pstmnt.setString(1, b.getNaam());
            pstmnt.executeUpdate();
